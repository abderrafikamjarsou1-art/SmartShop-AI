...
describe("GET /api/notifications", () => {
  ...
  it("regression: an explicit unreadOnly=false in the query string stays false (mobile 'All' tab bug)", async () => {
    vi.mocked(requireBusiness).mockResolvedValue(ctx);
    vi.mocked(notificationService.list).mockResolvedValue({
      items: [{ id: "n1" }, { id: "n2" }], total: 2, page: 1, totalPages: 1, unreadCount: 1,
    } as never);

    await GET(jsonRequest("http://localhost/api/notifications?unreadOnly=false", "GET"));

    expect(notificationService.list).toHaveBeenCalledWith(ctx, expect.objectContaining({ unreadOnly: false }));
  });
