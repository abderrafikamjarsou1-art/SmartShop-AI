...
/**
 * POST /api/purchases/[id]/cancel. purchaseService.cancel() refuses this
 * once any line has been received (ValidationError) — not re-implemented
 * here.
 */
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const purchase = await purchaseService.cancel(ctx, zParse(zUuid, id));
    return ok(purchase, requestId);
  } catch (error) {
    return fail(error, requestId, "POST /api/purchases/[id]/cancel");
  }
}
